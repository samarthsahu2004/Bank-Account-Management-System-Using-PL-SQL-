...
-- Step 3: Insert Sample Data (~15 entries)
-- =====================================================
...
-- Trigger to Auto-update Balance (Alternative approach - logging trigger)
-- Note: Balance is updated in procedures, but this trigger ensures consistency

-- Trigger to Log Transactions (Additional logging for audit)
CREATE OR REPLACE TRIGGER trg_balance_update
AFTER UPDATE OF balance ON accounts
FOR EACH ROW
WHEN (OLD.balance != NEW.balance)
BEGIN
    -- This trigger ensures transaction logging when balance changes
    -- The actual transaction record is created in procedures, but this provides audit trail
    NULL; -- Balance updates are handled in procedures with explicit transaction logging
END;
/

-- Trigger to prevent negative balance (safety check)
...
-- Example Usage:
/*
-- 1. Create a new customer and account
DECLARE
    v_cust_id NUMBER;
    v_acc_id NUMBER;
